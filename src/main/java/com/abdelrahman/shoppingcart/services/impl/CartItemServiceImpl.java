...
public class CartItemServiceImpl implements CartItemService {
	...
	@Override
	@Transactional
	public Cart addItemToCart(Long productId,Long cartId,int quantity) {
		// creating cart 
	...
	@Override
	@Transactional
	public Cart updateItemQuantity(Long cartId, Long productId, int newQuantity) {
		Cart cart = cartService.getOrCreateCart(cartId);
		Product product = productService.getProduct(productId);
		if(newQuantity <= 0) {
		    throw new IllegalArgumentException("Quantity must be greater than 0");
...
//		cartRepo.save(cart);
	...
	@Override
	@Transactional
	public Cart removeItemFromCart(Long cartId, Long itemId, int removedCount) {
